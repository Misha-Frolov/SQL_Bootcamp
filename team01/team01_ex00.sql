WITH r_currency AS (SELECT id, MAX(updated) AS updated FROM currency
    GROUP BY id),
    s_currency AS (SELECT *FROM currency AS c
...
FULL JOIN s_balance AS b
    ON b.user_id = u.id
FULL JOIN s_currency AS sc 
    ON sc.id = b.currency_id
